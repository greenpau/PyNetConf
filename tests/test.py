...
try:
    ...
    for e in err.args:
        print('%-10s >>   %s: %s' % ('global()', str(type(err).__name__), str(e)));
    sys.exit(1);
...
def main():    
    ...
    descr += 'examples:\n';
    descr += '  python3 tests/test.py -ho router -u admin -p Y2lzY28K -l 5\n';
    descr += '    [note: "Y2lzY28K" is base64-encoded "cisco"]\n';
    descr += '  python3 tests/test.py -ho router -u admin -k ~/.ssh/id_rsa -l 5\n';
    ...
    try:
        ...
        nc.show('client-capabilities');    # alternatively, pprint.pprint(nc.capabilities['client']);  
        nc.show('server-capabilities');    # alternatively, pprint.pprint(nc.capabilities['server']);  
        ...
        cmds = ['show run', 'show version', 'show interfaces'];
        for c in cmds:
            nc.cmd(c);
            if nc.resp:
                nc.show('response');
        ...
        for e in err.args:
            print('%-10s >>   %s: %s' % (func, str(type(err).__name__), str(e)));
        if args.ilog == 5:
